...
const AnnualBonus = ({prevStep, nextStep, handleFormData, values: value }) => {
  const {
    values,
    errors,
    handleBlur,
    ...
    onSubmit: (formValues) => {
      handleFormData({
        ... value,
        annualBonus: formValues.annualBonus,
      });
      nextStep();
    },
  });
   ...
   animate={{ x:"calc(50% - 50%)"  }}>
      <Form >
        <Box style={{ marginTop: 100 }}>
          <h4 style={{ fontFamily: "Corbel" }}>Total annual bonus(optional)</h4>
          <Grid
            container
            spacing={2}
            alignItems="center"
          ...
          >
            <Grid item xs={12}>
              ...
              <Input
              name="annualBonus"
                value={values.annualBonus}
                size="small"
                onChange={(e) => setFieldValue("annualBonus",e.target.value)}
                onBlur={handleBlur}
                inputProps={{
                  step: 10,
                  min: 0,
            ...
            <Grid item xs={12} style={{ marginTop: "10px" }}>
              <PrettoSlider
                value={values.annualBonus || 0}
                marks={IncomeSlider.marksAmt}
                max={200000}
                onChange={(e,newValue) => setFieldValue("annualBonus", newValue)}
                aria-labelledby="input-slider"
              ></PrettoSlider>
            </Grid>
          </Grid>
